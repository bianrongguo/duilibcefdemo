...
#endif
{
	...
	StringCchPrintf(szDirectoryDump, _countof(szDirectoryDump), _T("%s\\WeiKe"), tszModule);
	if (!PathFileExists(szDirectoryDump))
	{
		CreateDirectory(szDirectoryDump, NULL);
	}
	StringCchPrintf(szDirectoryLog, _countof(szDirectoryLog), _T("%s\\WeiKe\\%s"), tszModule, WSP_LOG_DIR);
	...
	StringCchPrintf(m_szLogFileName, countof(m_szLogFileName),
		L"%s\\WeiKe\\%s\\%s.log", tszModule, WSP_LOG_DIR, logname);
}
