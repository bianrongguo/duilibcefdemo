...
std::wstring getExeVersion(std::wstring iniPath)
{
	TCHAR verLExe[MAX_PATH] = { 0 };
	GetPrivateProfileString(_T("version"), _T("version"), 0, verLExe, MAX_PATH, iniPath.c_str());
	std::wstring exeVersion = verLExe;
	return exeVersion;
}
...
CMiniDumper::CMiniDumper( bool bPromptUserForMiniDump )
{
	...
	memset(m_szMiniDumpPath, 0, sizeof(TCHAR)* MAX_PATH);
}
...
void CMiniDumper::setMiniDumpFilePath(LPCTSTR lpFilePath)
{
	if (lpFilePath == NULL)
	{
		return;
	...
	{
		CCommonInterface CommonInterface;
		CommonInterface.CreateDir(lpFilePath);
	}
	time_t currentTime;
	time(&currentTime);
	std::wstring strClientVersionIniPath = m_szAppPath;
	strClientVersionIniPath += _T("config\\clientversion.ini");
	std::wstring version = getExeVersion(strClientVersionIniPath);
	...
	_tcssprintf(m_szMiniDumpPath,
		...
		version.c_str(),
		currentTime);
	//StringCchPrintf(m_szMiniDumpPath, _countof(m_szMiniDumpPath), _T("%s"), lpFilePath);
}
//-----------------------------------------------------------------------------
...
{
	//如果已经设置过了就不用再设置了
	if (lstrlen(m_szMiniDumpPath) > 0)
	{
		return;
	}
    time_t currentTime;
    time( &currentTime );
	///dump文件保存到缓存目录 2015年2月3日 by lixiaokang
   /* _tcssprintf( m_szMiniDumpPath,
                 _T( "%s%s.%ld.dmp" ),
                 m_szAppPath,
                 m_szAppBaseName,
	...
	SHGetSpecialFolderPath(
		...
		CSIDL_LOCAL_APPDATA,
		FALSE);
	std::wstring strAppLocal = szAppDataPath;
	StringCchCat(szAppDataPath, _countof(szAppDataPath), _T("\\Haoti\\crashdump\\"));
...
LONG CMiniDumper::writeMiniDump( _EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	std::wstring strpath = m_szAppPath;
	strpath += _T("bugReporter.exe");
	ShellExecute(0, _T("open"), strpath.c_str(), _T(""), _T(""), SW_SHOWNORMAL);
