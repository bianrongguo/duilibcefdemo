...
DUI_BEGIN_MESSAGE_MAP(WindowImplBase,CNotifyPump)
	...
	DUI_ON_MSGTYPE(DUI_MSGTYPE_SIZECHANGED, OnNotifySizeChanged)
DUI_END_MESSAGE_MAP()
...
LRESULT WindowImplBase::ResponseDefaultKeyEvent(WPARAM wParam)
{
	...
	{
		//Close(); //2014年12月1日 by lixiaoknag ;程序不相应esc，当登陆/注册等窗口的文本框获取焦点再按esc，登陆/注册窗口获取不到esc消息，Close（）函数会把主窗体关闭，登陆/注册框不会关闭；
		return TRUE;
	}
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	ASSERT(pRoot);
	if (pRoot==NULL)
	{//chg by lch 2014-6-10
		//MessageBox(NULL,_T("加载资源文件失败"),_T("Duilib"),MB_OK|MB_ICONERROR);
		GetLogFun(Error)(_T("Create xml= %s filed!"), GetSkinFile().GetData());
		//dangerous use this 
		//ExitProcess(1);
		return 0;
	}
...
void WindowImplBase::OnNotifySizeChanged(TNotifyUI& msg)
{
	return;
}
