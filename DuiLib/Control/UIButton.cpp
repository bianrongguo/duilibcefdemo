...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_KEYDOWN )
		{
			if (IsKeyboardEnabled()) {
				...
				// zhangcl 2015-07-27 屏蔽了上面的回车响应，单独进行处理。
				else if (event.chKey == VK_RETURN)
				{
					responseEnterKey();
					return;
				}
		...
		//chg the cursor type to IDC_ARROW
			if (m_isHandCursor)
			{
	...
	bool CButtonUI::responseEnterKey()
	{
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("pushedfont")) == 0)
		{
			SetPushedFont(_ttoi(pstrValue));
		}
		//============================================================================
		// 设置小手
		else if (_tcscmp(pstrName, _T("handcursor")) == 0)
		{
	...
	int CButtonUI::GetPushedFont()
	{
		return m_iPushedFont;
	}

	void CButtonUI::setHandCursor(bool isSet)
	{
		m_isHandCursor = isSet;
	}
