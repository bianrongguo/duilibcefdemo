...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		// zhangcl 2015-02-28  CEditUI添加提示文本
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditTextColor(0x00000000), m_dwTipValueColor(0xFFBAC0C5)
	{
	...
	void CEditUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_SETFOCUS && IsEnabled() ) 
		{
			...
			ASSERT(m_pWindow);
			m_pWindow->m_bCLIPSIBLINGS = m_bEnableClipsiblings;
			m_pWindow->Init(this);

			// zhangcl 2015-02-28 Edit控件切换焦点之后将焦点放到最后一个字符的后面
			SetSelAll();
			SetReplaceSel(GetText());
			//==================================================================
