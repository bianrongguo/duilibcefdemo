...
static void adjustBottomOfPopupWindow(
    RECT &popupWindowArea, SIZE &availableSize, CMenuElementUI* pOwner);
...
LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if( m_pOwner != NULL) 
	{
		...
		// zhangcl 添加滚动条
		m_pLayout->SetAttribute(_T("vscrollbar"), _T("true"));
		//=====================================================================================
		...
		{
            adjustBottomOfPopupWindow(rc, szAvailable, m_pOwner);
            rc.bottom = rc.top;
            rc.top = rc.bottom - cyFixed;
        }
		...
		//菜单最底部不能低于工作区
		if (rc.bottom > rcWork.bottom && 
			(rc.bottom - rc.top) <= rcWork.GetHeight())
		{
			//通过逐步移动的方法向上移动
		...
		//end add
		if ((rc.bottom - rc.top) <= rcWork.GetHeight())
		{
		...
		{
			MoveWindow(m_hWnd, rc.left, 0, rc.right - rc.left, rcWork.GetHeight(), FALSE);
		}
	}
	// 添加第一级菜单
	else 
	{
		...
		if (m_dwAlignment == 0)
		{
			if (rc.left - rc.GetWidth() < rcWork.left || rc.bottom - rc.GetHeight() > rcWork.bottom)
			{
				m_dwAlignment = 0;
				m_dwAlignment = eMenuAlignment_Right | eMenuAlignment_Top;
			}
			else if (rc.right + rc.GetWidth() > rcWork.right || rc.bottom + rc.GetHeight() > rcWork.bottom)
			{
				m_dwAlignment = 0;
				m_dwAlignment = eMenuAlignment_Top | eMenuAlignment_Left;
			}
			else if (rc.top - rc.GetHeight() < rcWork.top || rc.right + rc.GetWidth() > rcWork.right)
			{
				m_dwAlignment = 0;
				m_dwAlignment = eMenuAlignment_Bottom | eMenuAlignment_Left;
			}
			else if (rc.top - rc.GetHeight() < rcWork.top || rc.left - rc.GetWidth() < rcWork.top)
			{
				m_dwAlignment = 0;
...
{
	if (event.Type == UIEVENT_MOUSEENTER || (event.Type == UIEVENT_KEYDOWN && (event.chKey == VK_RIGHT)))
	{
	...
	else if( event.Type == UIEVENT_BUTTONUP || (event.Type == UIEVENT_KEYDOWN && event.chKey == VK_RETURN) )
	{
		if( IsEnabled() ){
			CListContainerElementUI::DoEvent(event);
			...
			else
			{
				...
				if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
				{
					...
					// zhangcl 2014-04-22 这里修改成发送的参数是本对象的指针
					PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)(this), (LPARAM)GetChecked());
				}
				ContextMenuParam param;
				param.hWnd = m_pManager->GetPaintWindow();
				param.wParam = 1;
...
void CMenuElementUI::SetAllChildEnVisible()
{
	for (int i = 0; i < GetCount(); ++i)
	{
		if (GetItemAt(i)->GetInterface(_T("MenuElement")) != NULL)
		{
			(static_cast<CMenuElementUI*>(GetItemAt(i)->GetInterface(_T("MenuElement"))))->SetVisible(false);
			(static_cast<CMenuElementUI*>(GetItemAt(i)->GetInterface(_T("MenuElement"))))->SetInternVisible(false);
		}
	}
}

void adjustBottomOfPopupWindow(RECT &popupWindowArea, SIZE &availableSize, CMenuElementUI* pOwner)
{
    // recalculate the area of the popup window in absolute space
    int iLastItem = pOwner->GetCount() - 1;
    if (0 <= iLastItem)
    {
        CControlUI* pControl = pOwner->GetItemAt(iLastItem);
        if (pControl->GetInterface(_T("MenuElement")) != NULL)
        {
            SIZE sz = pControl->EstimateSize(availableSize);
            popupWindowArea.bottom += sz.cy;
            popupWindowArea.top += sz.cy;
        }
    }
}
