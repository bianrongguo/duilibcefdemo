...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_KEYDOWN:
        ...
        case VK_UP:
			...
			{
				SelectItem(FindSelectable(GetCount(), false), true);
			}
            return;
        case VK_DOWN:
			if (m_iCurSel + 1 < GetCount())
			{
				SelectItem(FindSelectable(m_iCurSel + 1, true), true);
			}
			else
			{
				SelectItem(FindSelectable(0, true), true);
			}
            return;
        case VK_PRIOR:
            PageUp();
