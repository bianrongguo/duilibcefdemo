...
namespace DuiLib
{
	...
	void CTreeNodeUI::DoEvent( TEventUI& event )
	{
		...
		{
			if( IsEnabled() ) {
				//当没有子节点时双击不再展开，按单击处理 lixiaokang 2015年4月29日
			/*	CTreeNodeUI* pNode = GetFirstCTreeNodeUIFromPoint(event.ptMouse);
				if (pNode != NULL&&!pNode->IsHasChild())
				{
		...
		// zhangcl 2015-07-28 处理键盘左键收起
		else if (event.Type == UIEVENT_KEYDOWN && event.chKey == VK_LEFT)
		{
			if (IsEnabled())
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_LEFT);
				Invalidate();
			}
			return;
		}
		// zhangcl 2015-07-28 处理键盘右键展开
		else if (event.Type == UIEVENT_KEYDOWN && event.chKey == VK_RIGHT)
		{
			if (IsEnabled())
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_RIGHT);
				Invalidate();
			}
			return;
		}
	...
	// 函数说明: 通过节点对象添加节点
	//************************************
	bool CTreeNodeUI::Add(CControlUI* _pTreeNodeUI, bool addDelegate)
	{
		if (_tcsicmp(_pTreeNodeUI->GetClass(), _T("TreeNodeUI")) == 0)
			return AddChildNode((CTreeNodeUI*)_pTreeNodeUI, addDelegate);
	...
	// 函数说明: 
	//************************************
	bool CTreeNodeUI::AddAt(CControlUI* pControl, int iIndex, bool addDelegate)
	{
		...
		if(pTreeView && pIndexNode)
			return pTreeView->AddAt((CTreeNodeUI*)pControl, pIndexNode, addDelegate);
		else 
	...
	// 函数说明: 
	//************************************
	bool CTreeNodeUI::AddChildNode(CTreeNodeUI* _pTreeNodeUI, bool addDelegate)
	{
		if (!_pTreeNodeUI)
			return false;
		...
		if(pTreeView){
			...
			if(!pNode || !pNode->GetLastNode())
				nRet = pTreeView->AddAt(_pTreeNodeUI, GetTreeIndex() + 1, addDelegate) >= 0;
			else nRet = pTreeView->AddAt(_pTreeNodeUI, pNode->GetLastNode()->GetTreeIndex() + 1, addDelegate) >= 0;
		}
	...
	///拖动节点使用 by lxk 2015年4月13日
	//begin
	void CTreeNodeUI::AddNodeFromList(CStdPtrArray &dstList,bool addDelegate)
	{
		for (int i = 0; i<dstList.GetSize(); i++)
		{
			MoveNode* moveNode = static_cast<MoveNode*>(dstList.GetAt(i));
			AddChildNode(moveNode->pNode,addDelegate);
			moveNode->pNode->pParentTreeNode = this;
			moveNode->pNode->AddNodeFromList(moveNode->childList, addDelegate);
			delete moveNode;
		}
	...
	//扩展treenode
	void CTreeNodeUI::SetTreeNodeExpandControl(CControlUI* pControl)
	{
		if (pControl != NULL)
		{
			pControl->SetFixedHeight(GetFixedHeight());
			pControlExpand = pControl;
			pHoriz->Add(pControl);
		}
	}

	//获取扩展的item控件
	CControlUI* CTreeNodeUI::GetItemExpandControl()
	{
		return pControlExpand;
	}
	/*****************************************************************************/
	/*****************************************************************************/
	/*****************************************************************************/
	...
	//************************************
	long CTreeViewUI::AddAt(CTreeNodeUI* pControl, int iIndex, bool addDelegate)
	{
		if (!pControl)
			return -1;
	...
	// 函数说明:
	//************************************
	bool CTreeViewUI::AddAt(CTreeNodeUI* pControl, CTreeNodeUI* _IndexNode, bool addDelegate)
	{
		...
		return AddAt(pControl, nItemIndex, addDelegate) >= 0;
	}
	...
	bool CTreeViewUI::OnDBClickItem( void* param )
	{
		...
		else if (pMsg->sType == DUI_MSGTYPE_LEFT)
		{
			return handleLeftKeyMessage(pMsg);
		}
		else if (pMsg->sType == DUI_MSGTYPE_RIGHT)
		{
			return handleRightKeyMessage(pMsg);
		}
		return false;
	}

	bool CTreeViewUI::handleLeftKeyMessage(void * param)
	{
		TNotifyUI* pMsg = (TNotifyUI*)param;
		if (pMsg->sType == DUI_MSGTYPE_LEFT)
		{
			CTreeNodeUI * pItem = static_cast<CTreeNodeUI*>(pMsg->pSender);
			CCheckBoxUI * pFolder = pItem->GetFolderButton();
			if (pFolder->IsEnabled() == false)
			...
			pFolder->Selected(true);
			pItem->SetVisibleTag(false);
			SetItemExpand(false, pItem);
			return true;
		}
		return false;
	}

	bool CTreeViewUI::handleRightKeyMessage(void * param)
	{
		TNotifyUI* pMsg = (TNotifyUI*)param;
		if (pMsg->sType == DUI_MSGTYPE_RIGHT)
		{
			CTreeNodeUI * pItem = static_cast<CTreeNodeUI*>(pMsg->pSender);
			CCheckBoxUI * pFolder = pItem->GetFolderButton();
			if (pFolder->IsEnabled() == false)
			...
			pFolder->Selected(false);
			pItem->SetVisibleTag(true);
			SetItemExpand(true, pItem);
			return true;
		}
		return false;
	}
	...
	void CTreeViewUI::Move(CTreeNodeUI* dstParent, CTreeNodeUI* pNode,int state)
	{
	...
	void CTreeViewUI::BeginDrag(CTreeNodeUI* pNode)
	{
		...
		static_cast<CContainerUI*>(m_pDragingCtrl)->FindSubControl(_T("labelNodeText"))->SetText(m_pNodeNeedMove->GetItemText());
	}
	...
	void CTreeViewUI::EndDrag(CTreeNodeUI* dstParent)
	{
		...
		if (m_pNodeNeedMove != NULL && dstParent != NULL && m_pNodeNeedMove != dstParent)
		{
			...
			Move(dstParent, m_pNodeNeedMove,2);
	...
	void CTreeViewUI::MouseLeaveNode()
	{
