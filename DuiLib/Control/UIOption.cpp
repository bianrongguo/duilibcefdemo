...
namespace DuiLib
{
	...
	void COptionUI::Selected(bool bSelected)
	{
		...
		if (m_bSelected)
		{
			SetBorderColor(m_dwSelectedBorderColor);
		}
		else
		{
			SetBorderColor(m_dwBorderColorbakSecected);
		}
		...
		else
		{
			if (event.Type == UIEVENT_KEYDOWN)
			{
				...
				if (event.chKey == VK_UP)
				{
					...
					if (m_pManager != NULL)
					{
						m_pManager->SendNotify(this, DUI_MSGTYPE_UP);
					}
				}
				else if (event.chKey == VK_DOWN)
				{
					...
					if (m_pManager != NULL)
					{
						m_pManager->SendNotify(this, DUI_MSGTYPE_DOWM);
					}
				}
