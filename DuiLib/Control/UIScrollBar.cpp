...
namespace DuiLib
{
	...
	int CScrollBarUI::GetButton1Height()
	{
		return m_iButton1Height;
	}
	int CScrollBarUI::GetButton2Height()
	{
		return m_iButton2Height;
	}
	void CScrollBarUI::SetButton1Height(int iHeight)
	{
		m_iButton1Height = iHeight;
	}
	void CScrollBarUI::SetButton2Height(int iHeight)
	{
		m_iButton2Height = iHeight;
	}
	...
	void CScrollBarUI::SetPos(RECT rc)
	{
		...
		if( m_bHorizontal ) {
			...
			if (m_iButton1Height == 0)
			{
				if (m_bShowButton1) cy -= m_cxyFixed.cx;
			}
			else
			{
				if (m_bShowButton1) cy -= m_iButton1Height;
			}
			if (m_iButton2Height == 0)
			{
				if (m_bShowButton2) cy -= m_cxyFixed.cx;
			}
			else
			{
				if (m_bShowButton2) cy -= m_iButton2Height;
			}
			if( cy > m_cxyFixed.cx ) {
				m_rcButton1.left = rc.left;
				m_rcButton1.top = rc.top;
				if( m_bShowButton1 ) {
					...
					{
						m_rcButton1.bottom = rc.top + m_iButton1Height;
					}
				}
				else {
					m_rcButton1.right = m_rcButton1.left;
				...
				if( m_bShowButton2 ) {
					...
					{
						m_rcButton2.top = rc.bottom - m_iButton2Height;
					}
	...
	int CScrollBarUI::WhichToShow(int pos)
	{	
		//add by lch for scroll message handle
		//2014-04-12
		int index=0;
		...
		else if (pos > 120 && pos <= 290)
		{
			index = 1;
		}
		else if (pos > 290 && pos <= 400)
		{
			index = 2;
		}
		else
	...
	void CScrollBarUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			//add by lch for scroll message handle
			//2014-04-12
			HWND hwnd = ::FindWindow(_T("HTZSsettingsWnd"), NULL);
			::SendMessage(hwnd, WM_USER_SETTINGWINDOWSCROLL, WhichToShow(m_nScrollPos), 0);
			{
				DuiLib::CDuiString layoutName = GetParent()->GetName();
				if (layoutName == _T("switchXTVertical"))
				{
					HWND hwnd = ::FindWindow(_T("XTRetrivalDialogWndClass"), _T("HTZSXTRetirevalDialog"));
					if (hwnd)
					{
						::SendMessage(hwnd, WM_USER_RETRIEVALWINDOWSCROLL, 0, 0);
					}
				}
			}
		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					...
					int vRange = m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom + m_rcThumb.top - 2 * m_cxyFixed.cy;
					...
					int hRange = m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - 2 * m_cxyFixed.cx;
		...
		if( event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID )
		{
			...
			::SendMessage(hwnd, WM_USER_SETTINGWINDOWSCROLL, WhichToShow(m_nScrollPos), 0);
			{
				...
				if (layoutName == _T("switchXTVertical"))
				{
					...
					if (hwnd)
					{
						::SendMessage(hwnd, WM_USER_RETRIEVALWINDOWSCROLL, 1, 0);
					}
				}
			}
			return;
	...
	void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("button1height")) == 0) SetButton1Height(_ttoi(pstrValue));
		else if (_tcscmp(pstrName, _T("button2height")) == 0) SetButton2Height(_ttoi(pstrValue));
