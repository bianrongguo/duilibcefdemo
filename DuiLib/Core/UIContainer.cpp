...
namespace DuiLib
{
	...
	void CContainerUI::DoEvent(TEventUI& event)
	{
		...
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled() )
		{
			...
			case SB_LINEUP:
				LineUp();
				//处理选题检索框滚动的效果 add by liulong 2014/10/17
				{
					DuiLib::CDuiString layoutName = GetName();
					if (layoutName == _T("switchXTVertical"))
					{
						HWND hwnd = ::FindWindow(_T("XTRetrivalDialogWndClass"), _T("HTZSXTRetirevalDialog"));
						if (hwnd)
						{
							int i = WM_USER_RETRIEVALWINDOWSCROLL;
				...
				if (m_pVerticalScrollBar)
				{
					...
					::SendMessage(hwnd, WM_USER_SETTINGWINDOWSCROLL, m_pVerticalScrollBar->WhichToShow(m_pVerticalScrollBar->GetScrollPos()), 0);
				}
				return;
			case SB_LINEDOWN:
				LineDown();
